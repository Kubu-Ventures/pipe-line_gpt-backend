...
**Ingestion** (`routers/ingest.py` → `tasks/celery_app.py`): upload validates the size while streaming, the extension allow-list, and PDF/ZIP magic bytes. It creates a `Document` row and dispatches `tasks.ingest_document` with the content **base64-encoded** in the Celery payload. Each task runs in a fresh `asyncio.run` loop, so `_ingest_async` builds a **per-task NullPool engine** (never cache an engine or async client across tasks). Retries first delete the document's existing chunks. `DocumentParseError` (bad file) fails without retrying, while other errors retry with backoff. Loaders return `(chunks, meta)`. Afterwards come `services/insights.extract_document_insights` (Claude → `Document.insights_json`, feeding `/dashboard/insights`), the audit events `INGEST_COMPLETED`/`INGEST_FAILED`, and cache invalidation.

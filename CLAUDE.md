...
- **`deploy/` is what customers run:** a compose stack with Caddy (TLS) routing `/` to the frontend and `/backend/*` to the API, so the frontend image is domain-agnostic. Postgres and Redis sit on an `internal` network. It also holds the `install.sh`, `backup.sh`, `restore.sh` and `upgrade.sh` scripts, and `README.md`, the operator guide. `install.sh` also runs unattended when its answers are set as environment variables (listed at its top).
- **`ops/pilots/provision.sh`** sets up hosted pilots on VMs over SSH, using `install.sh` unattended and `create-admin --password-stdin`. It is not in the customer bundle. Records (admin password, server `.env`) go to `~/.pipelinegpt-pilots/<name>/`, never into the repo.
